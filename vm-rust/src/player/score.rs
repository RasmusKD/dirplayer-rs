            ...
            let measured_height = if text_width > 0 {
                ...
                from_bitmap.or_else(|| {
                    ...
                    let per_line = if text_member.fixed_line_space > 0 {
                        ...
                        // The renderer steps lines by the font's measured
                        // line height; size the box by the same stride so
                        // the last line is not cut short.
                        let measured = crate::player::font::system_line_height_px(
                            &text_member.font, fs as f64,
                        );
                        if measured > fs as f64 {
